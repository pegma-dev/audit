...
const packageVersion = (
  JSON.parse(
    readFileSync(
      join(process.cwd(), "packages", "audit", "package.json"),
      "utf8",
    ),
  ) as { version: string }
).version;
...
describe("release package metadata", () => {
  ...
  it("keeps the exact public package inventory", () => {
    expect(RELEASE_PACKAGES.map(({ name }) => name)).toEqual(["@pegma/audit"]);
  });
...
describe("release source authentication", () => {
  it("accepts only an approved signed annotated tag at the event commit", () => {
    const root = mkdtempSync(join(tmpdir(), "audit-release-tag-"));
    try {
